...
def get_scores(loader):
    model.eval()
    total_output = np.array([999])
    total_output = np.reshape(total_output, (1, 1))
    for data in loader:
        data = data.to(device)
        pred = model(data)
...
path = "/your/path/to/models/lundnet_200_oneout_l10.pt"
model = LundNet()
...
model_name = "lundnet_200_output_l10"
#input TTree name
intreename = "lundjets_InDetTrackParticles"
...
for i_file, infile_name in enumerate(infiles_list):
    ...
    NBHadrons = pad_aux(tree["Akt10TruthJet_inputJetGABHadrons"].array(), 30)[:,0]
    #### 
    #For Full decluster
    ...
    print(y_pred)
    tagger_scores = np.squeeze(y_pred)
